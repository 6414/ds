        ...
        while True:
            def claimit():
                ...
                if ("ok") in claim.text:
                    ...
                    webhook = DiscordWebhook(url="https://discord.com/api/webhooks/904486568880914462/vFWaanUadcLK5v6XSLV7V8KxxPwonQzRQ_JWIs9IUT9XwfhnnVm1tw6NdF_cWsb00gy7")
                    embed = DiscordEmbed(title="Divine Just Swapped!", color=242424)
                    embed.add_embed_field(name="Link", value=f"https://instagram.com/{userid}")
                    embed.add_embed_field(name="Swap Type", value=f"{swap_type}")
                    embed.set_author(name="Swap Monitor")
                    embed.set_footer(text="Divine#3333")
                    embed.set_thumbnail(url='https://i.imgur.com/8YSXm2z.png')
                    
                    
                    embed.set_footer(text="[Developed By Divine]")
